            ...
            <div className="card-wrapper w-[90%] sm:w-full mt-5 flex md:flex-col sm:gap-5 mx-auto ">
                <div className="left flex-1 flex justify-center">
                    <fieldset data-aos="zoom-in" className=" w-[80%] p-5 py-12 sm:py-8 sm:w-full sm:p-2">
                        {personalDataObj.certificate.length > 0 && <>
                            <legend className=" w-auto ml-[50%] translate-x-[-50%] border-2 border-gray-200 rounded-3xl py-1 px-8 font-semibold text-xl text-yellow-500">
                                Certificate
                            </legend>
                            {personalDataObj.certificate.map((item, index) => {
                                return <Card index={index} key={index} heading={item.cerName} insName={item.institute} time={item.time} description={item.description} />
                            })}
                        </>}
                ...
                <div className="right flex-1 flex justify-center">
                    <fieldset data-aos="zoom-in" className=" w-[80%] p-5 py-12 sm:py-8 sm:w-full sm:p-2">
                        ...
                        {personalDataObj.education.map((item, index) => {
                            return <Card index={index} key={index} heading={item.course} insName={item.college} time={item.time} description={item.description} />
                        })}
...
const Card = ({ heading, insName, time, description ,index}) => {

    return <>
        <div className={`${index == 0 ? '' : 'mt-2'} relative`}   >
